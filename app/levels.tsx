...
import { getLevels, worldNames } from '../src/levels';
...
export default function Levels() {
  ...
  return (
    <Shell>
      ...
      {worldNames.map((name, world) => {
        const section = levels.slice(world * 4, world * 4 + 4),
          done = section.every((l) => l.complete);
        return (
          <View
            key={name}
            style={[
          ...
          >
            <View style={[s.row, { justifyContent: 'space-between' }]}>
              <View>
                <Text style={s.eyebrow}>
                  {t('WORLD {number}', { number: world + 1 })}
                </Text>
                <Text style={[s.section, { marginTop: 5 }]}>{t(name)}</Text>
              </View>
                  ...
                  >
                    ...
                    >
                      ...
                      <Pressable
                        ...
                        disabled={!progress.ready}
                        onPress={() =>
                          router.push(`/lesson/${level.nextWord.id}`)
                        }
                        style={({ pressed }) => [
                          styles.node,
                          {
            ...
            >
              {done
                ? t('World complete!')
                : t('Four letters. A new discovery at every stop.')}
            </Text>
          </View>
        );
      })}
