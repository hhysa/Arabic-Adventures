...
const paintNames: Record<string, string> = {
  '#E98B38': 'Orange color',
  '#D55B5B': 'Red',
  '#78A857': 'Green',
  '#55A6CE': 'Blue',
...
export default function Coloring() {
  ...
  const { id } = useLocalSearchParams<{ id: string }>(),
    ...
    active = useRef<DrawingGesture | null>(null),
    chosen = useRef(color);
  ...
  return (
    <Shell scrollEnabled={Platform.OS === 'web' || !drawing}>
      <Pressable onPress={() => router.back()}>
        <Text style={s.back}>{t('← Back to your word')}</Text>
      </Pressable>
      <Text style={s.title}>{t('A splash of imagination')}</Text>
      <Text style={s.sub}>
        {t(
          'Draw around your {word} and trace the Arabic word. Drag your finger or mouse to paint.',
          { word: t(lesson.english).toLocaleLowerCase() },
        )}
      </Text>
      <View
        style={{
      ...
      >
        <View
          ...
          collapsable={false}
          style={{ flex: 1, touchAction: 'none', userSelect: 'none' } as any}
        ...
        >
          ...
          >
            <Text style={{ fontSize: 140 }}>{lesson.emoji}</Text>
            <Text style={[s.arabic, { fontSize: 80 }]}>{lesson.arabic}</Text>
          </View>
      ...
      <View style={[s.row, { justifyContent: 'center' }]}>
        {Object.entries(paintNames).map(([c, name]) => (
          <Pressable
            key={c}
            accessibilityRole="button"
            accessibilityLabel={t('Paint {color}', { color: t(name) })}
      ...
      <View style={[s.row, { justifyContent: 'center' }]}>
        <Button
          secondary
          disabled={!strokes.length || drawing}
          onPress={() => setStrokes((v) => v.slice(0, -1))}
        >
          {t('Undo')}
        </Button>
        <Button
        ...
        <Button onPress={() => router.back()}>{t('All done ✓')}</Button>
      </View>
    </Shell>
  );
}
